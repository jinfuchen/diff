...
There exist more than hundred of metrics in our model. In addition, different projects have different features as the code tokens and configuration options are different. To make our results more generalizable, we measure the important features in the dimension level.
...
Finally, for each model with different dimensions of metrics, we have ten values of AUC since we use 10-fold cross validation. We compare all these models based on their respective AUC values.  In particular, we use the \emph{Mean Decrease Accuracy}~\cite{breiman2001random} to examine whether there is statistically significant difference between the original model with all dimensions of metrics and other models. %\bram{wouldn't RQ1 have already shown this, or why still necessary to do this analysis?}
